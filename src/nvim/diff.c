...
static void diff_check_cb(RStream *rstream, void *job, bool eof)
{
  bool *ok = job_data(job);
  if (!eof) {
    return;
  }
  char_u *buf = (char_u *) rbuffer_read_ptr(rstream_buffer(rstream));
  const char_u *end = buf + rstream_pending(rstream);
  for (const char_u *p = buf; p < end - 3 && ! *ok; p++) {
...
static void diff_read_cb(RStream *rstream, void *job, bool eof)
{
  struct diff_data *data = job_data(job);
  ...
  RBuffer *rbuf = rstream_buffer(rstream);
  char_u *buf = (char_u *) rbuffer_read_ptr(rbuf);
  char_u *end = buf + rstream_pending(rstream);
  char_u linebuf[LBUFLEN];  // only need to hold the diff line
  while (buf < end) {
    char_u *eol = xmemscan(buf, NL, end - buf);
    size_t line_len = (size_t)(eol - buf) + 1;  // Include the NL.
    if (line_len >= LBUFLEN) {
      line_len = LBUFLEN - 1;
    }
  ...
  rbuffer_consumed(rbuf, (char *) buf - rbuffer_read_ptr(rbuf));
